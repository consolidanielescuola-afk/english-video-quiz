"""
Costruzione del prompt e chiamata all'LLM (Anthropic Claude) per generare gli
...
Per usare OpenAI al posto di Anthropic: sostituire `_call_llm` con una
chiamata a `client.chat.completions.create(..., response_format={"type": "json_object"})`
mantenendo invariate le funzioni `build_prompt` e `generate_exercises`.
"""
...
# NB: verifica sempre l'ID modello più recente su https://docs.claude.com/en/docs/about-claude/models
...
def _call_llm(prompt: str, correction_note: str | None = None) -> str:
    ...
    if correction_note:
        messages.append({"role": "assistant", "content": "(risposta precedente non valida)"})
        messages.append({"role": "user", "content": correction_note})
